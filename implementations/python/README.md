...
Bump the manual version and add a comment. **Only works on the main branch.**
...
Bump manual version. **Only works on the main branch.**
...
```python
from verbeat import bump_version, VerBeatBranchError

try:
    new_version = bump_version("New feature")
    print(f"Bumped to version {new_version}")
